...
// Rewrite: card mutations now go via edge functions (useCards / cardActions).
// Math.random card-number generation removed entirely.
...
const EMPTY_FORM = {
  ...
  wallet_id: "",
  spending_limit: "5000",
  spend_period: "monthly" as "daily" | "monthly",
  card_type: "virtual",
...
export default function CardsPage() {
  ...
  const { cards: allCards, isLoading, issueCard, isIssuing } = useCards();
  ...
  const { data: transactions = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("transactions")
        .select("*, cards(card_name, last_four)")
        .eq("org_id", orgId!)
  ...
  const handleIssue = async (e: React.FormEvent) => {
    ...
    try {
      await issueCard({
        holderId: form.holder_id || user!.id,
        cardName: form.card_name,
        cardType: form.card_type as "virtual" | "physical",
        spendingLimit: parseFloat(form.spending_limit),
        spendPeriod: form.spend_period,
        walletId: form.wallet_id || undefined,
        allowedCategoryIds: form.allowed_category_ids,
        allowedCountries: form.country_mode === "all" ? [] : form.allowed_countries,
      });
      setOpen(false);
      setForm(EMPTY_FORM);
  ...
  return (
    <div className="p-6 lg:p-8 max-w-[1400px]">
              ...
              <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
                ...
                <form onSubmit={handleIssue} className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <Input value={form.card_name} onChange={(e) => setForm({ ...form, card_name: e.target.value })} required placeholder="e.g. Marketing Budget Card" />
                  </div>
                  <div className="space-y-1.5">
                    <Label>Issue to *</Label>
                    <Select value={form.holder_id} onValueChange={(v) => setForm({ ...form, holder_id: v })}>
                      <SelectTrigger><SelectValue placeholder="Select team member" /></SelectTrigger>
                      <SelectContent>{members.map((m: any) => { const p = m.profiles as any; const name = p ? `${p.first_name || ""} ${p.last_name || ""}`.trim() : "Unnamed"; return <SelectItem key={m.user_id} value={m.user_id}>{name} <span className="text-muted-foreground text-xs ml-1">({m.role})</span></SelectItem>; })}</SelectContent>
                    </Select>
                  </div>
                  <div className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1.5"><Label>Limit period</Label><Select value={form.spend_period} onValueChange={(v: "daily" | "monthly") => setForm({ ...form, spend_period: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="daily">Daily</SelectItem><SelectItem value="monthly">Monthly</SelectItem></SelectContent></Select></div>
                  </div>
                  <div className="space-y-1.5">
                    <Label>Card type</Label>
                    <Select value={form.card_type} onValueChange={(v) => setForm({ ...form, card_type: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="virtual">Virtual</SelectItem><SelectItem value="physical">Physical</SelectItem></SelectContent></Select>
                  </div>
                  {categories.length > 0 && (
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto border rounded-md p-3">
                        {categories.map((cat: any) => (<label key={cat.id} className="flex items-center gap-2 text-sm cursor-pointer"><Checkbox checked={form.allowed_category_ids.includes(cat.id)} onCheckedChange={() => toggleCategory(cat.id)} />{cat.name}</label>))}
                      </div>
                    </div>
                  )}
                  <div className="space-y-1.5">
                    <Label>Country restrictions</Label>
                    <Select value={form.country_mode} onValueChange={(v: "all" | "selected") => setForm({ ...form, country_mode: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All countries</SelectItem><SelectItem value="selected">Selected countries only</SelectItem></SelectContent></Select>
                  </div>
                  {form.country_mode === "selected" && (
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-1.5 max-h-48 overflow-y-auto border rounded-md p-3">
                        {COUNTRIES.map((c) => (<label key={c.code} className="flex items-center gap-2 text-sm cursor-pointer"><Checkbox checked={form.allowed_countries.includes(c.code)} onCheckedChange={() => setForm(prev => ({ ...prev, allowed_countries: prev.allowed_countries.includes(c.code) ? prev.allowed_countries.filter(x => x !== c.code) : [...prev.allowed_countries, c.code] }))} /><span className="truncate">{c.flag} {c.name}</span></label>))}
                      </div>
                    </div>
                  )}
                  <Button type="submit" className="w-full" disabled={isIssuing}>{isIssuing ? "Issuing…" : "Issue card"}</Button>
                </form>
              </DialogContent>
            </Dialog>
          ) : undefined
      ...
      />
      <div className="mt-4">
        <DataTable headers={["Card", "Holder", "Type", "Limit", "Wallet", "Status"]} isLoading={isLoading} isEmpty={cards.length === 0} emptyMessage="No cards found.">
          {cards.map((card: any) => (
            <tr key={card.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors cursor-pointer" onClick={() => setSelectedCard(card)}>
              <td className="px-4 py-3"><div className="flex items-center gap-3"><div className="w-9 h-6 rounded bg-gradient-to-br from-primary/80 to-primary flex items-center justify-center"><CreditCard className="h-3.5 w-3.5 text-primary-foreground" /></div><div><p className="text-sm font-medium">{card.card_name}</p><p className="text-xs text-muted-foreground font-mono">•••• {card.last_four}</p></div></div></td>
      ...
      <CardDetailPanel card={selectedCard} open={!!selectedCard} onOpenChange={(v) => { if (!v) setSelectedCard(null); }} getMemberName={getMemberName} />
    </div>
  );
}
