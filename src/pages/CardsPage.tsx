...
export default function CardsPage() {
  ...
  // Fetch org members for holder selection
  ...
  const createCard = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("cards").insert({
        org_id: orgId!,
        holder_id: form.holder_id || user!.id,
    ...
    <div className="p-6 lg:p-8 max-w-[1400px]">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Issue virtual and physical cards linked to wallets</p>
        </div>
        {isAdmin && (
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button size="sm"><Plus className="h-4 w-4 mr-1.5" />Issue card</Button>
            </DialogTrigger>
            <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
              ...
              <form onSubmit={(e) => { e.preventDefault(); createCard.mutate(); }} className="space-y-4">
                      ...
                      {members.map((m: any) => {
                        ...
                        return (
                          <SelectItem key={m.user_id} value={m.user_id}>
                            {name} <span className="text-muted-foreground text-xs ml-1">({m.role})</span>
                          </SelectItem>
                        );
                      })}
                    </SelectContent>
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-muted-foreground">Card spend will deduct from this wallet's balance.</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Input type="number" step="0.01" min="0" value={form.spending_limit} onChange={(e) => setForm({ ...form, spending_limit: e.target.value })} />
                  </div>
                  <div className="space-y-1.5">
                ...
                {categories.length > 0 && (
                  <div className="space-y-2">
                    <Label>Allowed spend categories</Label>
                    <p className="text-xs text-muted-foreground">Leave all unchecked to allow all categories.</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        {isLoading ? (
          ...
          <div className="text-sm text-muted-foreground">No cards issued yet.</div>
        ) : (
          cards.map((card: any) => (
            <Card key={card.id} className="relative overflow-hidden">
              <CardContent className="p-5">
                ...
                <p className="text-xs text-muted-foreground mt-1">{getMemberName(card.holder_id)}</p>
                <div className="flex justify-between mt-3">
                  <span className="text-xs text-muted-foreground capitalize">{card.card_type}</span>
                  <span className="text-xs text-muted-foreground">
                    {Number(card.spending_limit).toLocaleString("de-DE")} € / {card.spend_period}
                  </span>
                </div>
                {card.wallets?.name && (
                  <div className="mt-2">
                    <Badge variant="outline" className="text-xs">{card.wallets.name}</Badge>
                  </div>
                )}
              </CardContent>
            </Card>
