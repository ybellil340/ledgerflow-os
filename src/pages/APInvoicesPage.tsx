...
export default function APInvoicesPage() {
  ...
  const { data: allInvoices = [], isLoading } = useQuery({
    queryKey: ["ap-invoices", orgId],
    queryFn: async () => {
      const { data, error } = await supabase.from("ap_invoices").select("*, suppliers(name)").eq("org_id", orgId!).order("created_at", { ascending: false });
  ...
  const invoices = allInvoices
    ...
    .filter((i: any) => !search || i.invoice_number.toLowerCase().includes(search.toLowerCase()) || i.suppliers?.name?.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="p-6 lg:p-8 max-w-[1400px]">
      ...
      <div className="mt-4">
        <DataTable
          headers={["Invoice #", "Supplier", "Amount", "Issue Date", "Due Date", "Status"]}
          isLoading={isLoading}
        ...
        >
          {invoices.map((inv: any) => (
            <tr key={inv.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
              ...
              <td className="px-4 py-3 text-sm">{inv.suppliers?.name || "—"}</td>
