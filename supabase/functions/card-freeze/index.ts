// supabase/functions/card-freeze/index.ts
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (card.status !== "active") return err(409, `Card is already ${card.status}`);
    ...
    return ok({ cardId, status: "frozen" });
  } catch (e: any) {
    console.error("[card-freeze]", e);
