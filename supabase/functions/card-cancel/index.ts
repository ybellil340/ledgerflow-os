// supabase/functions/card-cancel/index.ts
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (card.status === "cancelled") return err(409, "Card is already cancelled");

    const provider = getBankProvider(card.provider ?? "mock");
    await provider.cancelCard(card.provider_card_id);
    ...
    return ok({ cardId, status: "cancelled" });
  } catch (e: any) {
    console.error("[card-cancel]", e);
